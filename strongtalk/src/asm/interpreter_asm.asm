;; (C) 1994  - 2019, The Strongtalk authors and contributors
...
magic		= 0FCFCFCFCh			; must be a smi_t
...
class		EQU esi-4
		...
		_smiKlass edx			; preload smi_t class
		test al, Mem_Tag		; check if smi_t
...
class		EQU esi-4
		...
		_advance_aligned 9
		test al, Mem_Tag		; check if smi_t
		jz SHORT _cache_miss		; if smi_t then it's a cache miss
		mov ecx, [class]		; get inline cache
		mov edx, [eax._klass]		; get recv class
...
;
; method must be a methodOop or a non-zombie NativeMethod (zombie nmethods must be filtered
...
_no_args:	mov eax, [receiver]
		_restore_ebx
		mov edx, [method]
		test edx, Mem_Tag		; test if methodOop or NativeMethod
