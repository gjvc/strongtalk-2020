...
TEST_F( OldSpaceTests, expandShouldIncreaseExpansionCount ) {
    ...
    theSpace->expand( ObjectHeapExpandSize * 1024 );
...
TEST_F( OldSpaceTests, oldSpaceMarkShouldRestoreTop ) {
    ...
    Oop          *oldTop   = theSpace->top();
    {
        ...
        ASSERT_TRUE( theSpace->allocate( 100, false ) != nullptr );
        ASSERT_EQ( freeSpace - ( 100 * OOP_SIZE ), theSpace->free() );
    }
    ASSERT_EQ( freeSpace, theSpace->free() );
    ASSERT_EQ( (const char *) oldTop, (const char *) theSpace->top() );
}
