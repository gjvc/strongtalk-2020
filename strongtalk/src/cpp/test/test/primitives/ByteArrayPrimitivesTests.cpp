...
ASSERT_EQ( ( const char * ) byteArrayClassHandle.
as_klass(),
( const char * ) result->
klass()
);
...
ASSERT_EQ( size, ByteArrayOop( result )
->
length()
);
ASSERT_EQ( ( const char * ) byteArrayClassHandle.
...
EXPECT_EQ( address, SMIOop( byteArrayPrimitives::alienGetAddress( alien ) )
->
value()
...
EXPECT_TRUE( alien
==
byteArrayPrimitives::alienSetAddress( address
.
as_oop(), alien
) ) << "Should return alien";
EXPECT_EQ( ( std::int32_t ) alien_byte_region,
...
checkMarkedSymbol( "wrong receiver", result,
...
checkMarkedSymbol( "iillegal state", result,
...
checkMarkedSymbol( "argument invalid", result,
vmSymbols::argument_is_invalid()
);
}
