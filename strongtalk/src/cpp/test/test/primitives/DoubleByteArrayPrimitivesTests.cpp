...
ASSERT_EQ( 10,
DoubleByteArrayOop(result)
->
length()
);
ASSERT_EQ( ( const char * ) dByteArrayClassHandle.
as_klass(),
( const char * ) result->
klass()
);
...
ASSERT_EQ( ( const char * ) classHandle.
as_klass(),
( const char * ) result->
klass()
);
...
EXPECT_EQ( markSymbol( vmSymbols::failed_allocation() ), result
) <<
unmarkSymbol( result )
->
as_string();
