...
EXPECT_EQ( vmSymbols::argument_has_wrong_type(), unmarkSymbol( result )
) << "Wrong symbol";
}
...
EXPECT_EQ( vmSymbols::argument_is_invalid(), unmarkSymbol( result )
) << "Wrong symbol";
}
...
EXPECT_EQ( smiOopFromValue( oopSize ), SystemPrimitives::oopSize()
) << "oopSize is wrong";
}
...
) << "Wrong expansion count";
}


TEST( SystemPrimitivesTests, nurseryFreeSpaceShouldReturnEdenFreeSpaceAsSMI
) {
ASSERT_EQ( Universe::new_gen
.eden()->free(),
SMIOop ( SystemPrimitives::nurseryFreeSpace() )
->
value()
);
}
...
->
is_mark()
) << "return should be marked";
EXPECT_EQ( markSymbol( vmSymbols::argument_has_wrong_type() ), pointer
) << "wrong symbol returned";
}
...
EXPECT_EQ( markSymbol( vmSymbols::argument_is_invalid() ), pointer
) << "wrong symbol returned";
}
...
EXPECT_EQ( markSymbol( vmSymbols::argument_has_wrong_type() ), result
) << "wrong symbol returned";
}
...
EXPECT_EQ( markSymbol( vmSymbols::argument_is_invalid() ), result
) << "wrong symbol returned";
}
