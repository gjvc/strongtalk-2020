...
ASSERT_EQ( 1, clonedNode->
byteCodeIndex()
);
ASSERT_EQ( 2, clonedNode->
args()
);
ASSERT_TRUE( clonedNode
->
expressionStack()
!= node->
expressionStack()
...
char buffer[2048];
sprintf( buffer,
"Too few expressions on stack for 0x%x: required %d, but got %d", node, 1, 0 );
...
EXPECT_EQ( 0, strncmp( "UncommonSend(1 arg)", buffer, 19 ) ) <<
buffer;
}
...
char expected[100];
sprintf( expected,
"UncommonSend(1 arg)                     ((UncommonSendNode*)%#lx)", node );
char buffer[100];
node->
print_string( buffer );

EXPECT_EQ( 0,
strcmp( expected, buffer
) ) <<
buffer;
}
...
bbIterator->
pregTable = new GrowableArray<PseudoRegister *>;
