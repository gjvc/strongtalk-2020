...
extern "C" doFn dispatch_table[static_cast<std::size_t>(ByteCodes::Code::NUMBER_OF_CODES )];
extern "C" doFn original_table[static_cast<std::size_t>(ByteCodes::Code::NUMBER_OF_CODES )];

doFn dispatch_table[static_cast<std::size_t>(ByteCodes::Code::NUMBER_OF_CODES )];
doFn original_table[static_cast<std::size_t>(ByteCodes::Code::NUMBER_OF_CODES )];
    ...
    if ( not in_return_mode() ) {
        ...
        for ( std::size_t i = 0; i < return_codes_size; i++ ) {
            ...
            if ( ByteCodes::single_step( code ) ) {
                dispatch_table[ static_cast<std::size_t>(code) ] = (doFn) StubRoutines::single_step_stub();
            } else {
                dispatch_table[ i ] = original_table[ i ];
            }
        }
...
void dispatchTable_init() {
    _console->print_cr( "%%system-init:  dispatchTable_init" );
