...
constexpr T _flag( const char *name, T default_value, const char *description ) {
    ...
    if constexpr ( std::is_same<T, int>::value ) {
        ...
        std::cout << " int [" << cv.name() << "] default value is [" << cv.value() << "], overridden value is [" << co.value() << "]" << std::endl;
    }
...
auto _BlockArgAdditionalAllowedInlineCost = _flag<int>( "BlockArgAdditionalAllowedInlineCost", 35, "additional allowed cost for each block arg" );
auto _BlockArgAdditionalInstrSize         = _flag<int>( "BlockArgAdditionalInstrSize", 150, "extra allowance (in instr bytes) for each block arg" );
...
auto _CodeSize                            = _flag<int>( "CodeSize", 20 * 1024, "size of code cache (in Kbytes)" );
...
auto _CompilerInstrsSize                  = _flag<int>( "CompilerInstrsSize", 50 * 1024, "max. size of NativeMethod instrs" );
auto _CompilerPCsSize                     = _flag<int>( "CompilerPCsSize", 15 * 1024, "max. size of relocation info info per NativeMethod" );
auto _CompilerScopesSize                  = _flag<int>( "CompilerScopesSize", 50 * 1024, "max. size of debugging info per NativeMethod" );
...
auto _CounterHalfLifeTime                 = _flag<int>( "CounterHalfLifeTime", 30, "time (in seconds) in which invocation counters decay by half" );
...
auto _EdenSize                            = _flag<int>( "EdenSize", 512, "size of eden (in Kbytes)" );
...
auto _EventLogLength                      = _flag<int>( "EventLogLength", 1000, "Length of internal event log" );
...
auto _HeapSweeperInterval                 = _flag<int>( "HeapSweeperInterval", 120, "Time interval (sec) between starting heap sweep" );
...
auto _InliningDatabasePruningLimit        = _flag<int>( "InliningDatabasePruningLimit", 3, "Min. number of nodes in inlining structure to qualify for database" );
auto _InvocationCounterLimit              = _flag<int>( "InvocationCounterLimit", 10000, "max. number of method invocations before (re-)compiling" );
auto _JumpTableSize                       = _flag<int>( "JumpTableSize", 8 * 1024, "size of jump table" );
auto _LRUDecayFactor                      = _flag<int>( "LRUDecayFactor", 2, "LRUDecayFactor" );
...
auto _LoopCounterLimit                    = _flag<int>( "LoopCounterLimit", 10000, "max. number of loop iterations before (re-)compiling" );
...
auto _MaxBlockInlineCost                  = _flag<int>( "MaxBlockInlineCost", 70, "max. cost of block method" );
auto _MaxBlockInstrSize                   = _flag<int>( "MaxBlockInstrSize", 450, "max. inline size (in instr bytes) of block method" );
auto _MaxCustomization                    = _flag<int>( "MaxCustomization", 10, "max. number of customized method copies to create" );
auto _MaxElementPrintSize                 = _flag<int>( "MaxElementPrintSize", 64, "Maximum number of elements to print" );
auto _MaxFnInlineCost                     = _flag<int>( "MaxFnInlineCost", 40, "max. cost of normal inlined method" );
auto _MaxFnInstrSize                      = _flag<int>( "MaxFnInstrSize", 300, "max. inline size (in instr bytes) of normal method" );
auto _MaxInterpretedSearchLength          = _flag<int>( "MaxInterpretedSearchLength", 10, "max. number of intrepreted stack frames to traverse searching for recompilee" );
auto _MaxNmInstrSize                      = _flag<int>( "MaxNmInstrSize", 12000, "max. desired size (in instr bytes) of an method" );
auto _MaxRecompilationSearchLength        = _flag<int>( "MaxRecompilationSearchLength", 10, "max. number of real stack frames to traverse searching for recompilee" );
auto _MaxRecursionUnroll                  = _flag<int>( "MaxRecursionUnroll", 2, "max. unrolling depth of recursive methods" );
auto _MaxTypeCaseSize                     = _flag<int>( "MaxTypeCaseSize", 3, "max. number of types in typecase-based inlining" );
auto _MemoizeBlocks                       = _flag<bool_t>( "MemoizeBlocks", true, "memoize (delay creation of) blocks" );
auto _MinBlockCostFraction                = _flag<int>( "MinBlockCostFraction", 50, "(in %) inline block if makes up more than this fraction of parent's cost" );
auto _MinInvocationsBeforeTrust           = _flag<int>( "MinInvocationsBeforeTrust", 100, "min. number of invocations required before trusting NativeMethod's PICs" );
auto _MinSendsBeforeRecompile             = _flag<int>( "MinSendsBeforeRecompile", 2000, "min number of sends a method must have performed before being recompiled" );
auto _NativeMethodAgeLimit                = _flag<int>( "NativeMethodAgeLimit", 2, "min. number of sweeps before NativeMethod becomes old" );
auto _NumberOfBlockAllocations            = _flag<int>( "NumberOfBlockAllocations", 0, "Number of allocated blocks" );
auto _NumberOfBytecodesExecuted           = _flag<int>( "NumberOfBytecodesExecuted", 0, "Number of bytecodes executed by interpreter (if tracing)" );
auto _NumberOfContextAllocations          = _flag<int>( "NumberOfContextAllocations", 0, "Number of allocated block contexts" );
auto _ObjectHeapExpandSize                = _flag<int>( "ObjectHeapExpandSize", 512, "Chunk size (in Kbytes) by which the object heap grows" );
auto _OldSize                             = _flag<int>( "OldSize", 3 * 1024, "initial size of oldspace (in Kbytes)" );
...
auto _PICSize                             = _flag<int>( "PICSize", 128, "size of PolymorphicInlineCache cache (in Kbytes)" );
...
auto _PrintProgress                       = _flag<int>( "PrintProgress", 0, "No. of compilations that cause a . to be printed out (0 means turned off)" );
...
auto _ProfilerNumberOfCompiledMethods     = _flag<int>( "ProfilerNumberOfCompiledMethods", 10, "Max. number of compiled methods to print" );
auto _ProfilerNumberOfInterpreterMethods  = _flag<int>( "ProfilerNumberOfInterpreterMethods", 10, "Max. number of interpreter methods to print" );
...
auto _ReservedCodeSize                    = _flag<int>( "ReservedCodeSize", 10 * 1024, "Maximum size of code cache (in Kbytes)" );
auto _ReservedHeapSize                    = _flag<int>( "ReservedHeapSize", 50 * 1024, "Maximum size for object heap in Kbytes" );
auto _ReservedPICSize                     = _flag<int>( "ReservedPICSize", 4 * 1024, "Maximum size of PolymorphicInlineCache cache (in Kbytes)" );
...
auto _StackPrintLimit                     = _flag<int>( "StackPrintLimit", 64, "Number of stack frames to print in VM-level stack dump" );
auto _StopInterpreterAt                   = _flag<int>( "StopInterpreterAt", 0, "Stops interpreter execution at specified bytecode number" );
auto _SurvivorSize                        = _flag<int>( "SurvivorSize", 64, "size of survivor spaces (in Kbytes)" );
...
auto _ThreadStackSize                     = _flag<int>( "ThreadStackSize", 512, "Size (in 1024) of each thread's stack" );
...
auto _TraceInterpreterFramesAt            = _flag<int>( "TraceInterpreterFramesAt", 0, "Trace interpreter frames at specified bytecode number" );
...
auto _UncommonAgeBackoffFactor            = _flag<int>( "UncommonAgeBackoffFactor", 4, "for exponential back-off of UncommonAgeLimit based on NativeMethod version" );
auto _UncommonInvocationLimit             = _flag<int>( "UncommonInvocationLimit", 10000, "min. number of invocations uncommon NativeMethod before recompiling it again" );
auto _UncommonRecompileLimit              = _flag<int>( "UncommonRecompileLimit", 5, "min. number of uncommon traps before recompiling" );
