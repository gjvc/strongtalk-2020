...
char *lookupTypeName( LookupType l ) {
    ...
    if ( isBitSet( static_cast<std::int32_t>(l), DirtySendBit ) )
        addFlag( hasFlag, name, "dirty " );
    if ( isBitSet( static_cast<std::int32_t>(l), OptimizedSendBit ) )
        addFlag( hasFlag, name, "optimized " );
    if ( isBitSet( static_cast<std::int32_t>(l), UninlinableSendBit ) )
