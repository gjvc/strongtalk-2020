...
void NativeCall::verify() {
    ...
    if ( *( uint8_t * ) instruction_address() not_eq instruction_code ) fatal( "not a call imm32" );
}
...
void NativeMov::verify() {
    ...
    if ( ( *( uint8_t * ) instruction_address() & ~register_mask ) not_eq instruction_code ) fatal( "not a mov reg, imm32" );
}
...
void NativeTest::verify() {
    ...
    if ( *( uint8_t * ) instruction_address() not_eq instruction_code ) fatal( "not a test eax, imm32" );
}
